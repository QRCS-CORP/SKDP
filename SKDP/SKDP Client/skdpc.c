...
void skdp_generate_server_key(skdp_server_key* skey, const skdp_master_key* mkey, const uint8_t kid[SKDP_KID_SIZE])
{
	...
	qsc_cshake_initialize(&kctx, SKDP_PERMUTATION_RATE, mkey->mdk, SKDP_MDK_SIZE, SKDP_CONFIG_STRING, SKDP_CONFIG_SIZE, kid, SKDP_SID_SIZE);
	...
	qsc_memutils_copy(skey->kid, kid, SKDP_SID_SIZE);
...
size_t skdp_packet_to_stream(const skdp_packet* packet, uint8_t* pstream)
{
	...
	if (packet != NULL && pstream != NULL)
	{
		...
		{
			qsc_memutils_copy((pstream + sizeof(uint8_t) + sizeof(uint32_t) + sizeof(uint64_t)), (const uint8_t*)&packet->message, packet->msglen);
...
void skdp_stream_to_packet(const uint8_t* pstream, skdp_packet* packet)
{
	...
	if (packet != NULL && pstream != NULL)
	{
		...
		if (packet->msglen <= SKDP_MESSAGE_MAX)
		{
			qsc_memutils_copy((uint8_t*)&packet->message, (pstream + sizeof(uint8_t) + sizeof(uint32_t) + sizeof(uint64_t)), packet->msglen);
		}
	}
}